...
export class DataStoreController {
	...
	@Post('append')
	async append(@Body() data: KeysValuesData, @Headers('useruuid') userUuid: string): Promise<any> {
		try {
	...
	@Post()
	async put(@Body() data: KeyValueData, @Headers('useruuid') userUuid: string): Promise<any> {
		try {
	...
	@Post('get')
	async get(@Body() data: KeyData, @Headers('useruuid') userUuid: string): Promise<any> {
		try {
	...
	@Post('remove')
	async remove(@Body() data: KeyData, @Headers('useruuid') userUuid: string): Promise<any> {
		try {
	...
	@UseInterceptors(FileInterceptor('file'))
	async attachFile(@UploadedFile() file,
			@Headers('useruuid') userUuid: string,
			@Headers('metadata') metadataJson: string,
			@Param('storageId') storageId: string,
			@Param('fileExtension') fileExtension: string): Promise<{file: string}> {
		try {
			const buffer: Buffer = file.buffer;
	...
	async setRules(@Body() setDataRules: StorageDataRules): Promise<void> {
		try {
			await this.dataStoreService.setDataRules(setDataRules.rules, setDataRules.storageId);
		} catch (e) {
			console.error(e);
			throw new HttpException(e.message, HttpStatus.INTERNAL_SERVER_ERROR);
		}
