...
export class DataStoreService {
  ...
  private async resolveStorage(storageId: string): Promise<StorageService> {
    ...
    if (!storage) {
      storage = this._storages[storageId] =
        this.moduleRef.resolve<StorageService>(StorageService);
    }
    return storage;
  }
  ...
  public async setDataRules(
    rulesConfigs: Array<{ cmd: string; bindKey: string }>,
    storageId: string,
  ): Promise<void> {
  ...
  public async replaceMacros(data: {
