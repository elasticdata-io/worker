...
export class StorageService {
  ...
  private async _upsertLine(
    context: string,
    key: string,
    value: string,
  ): Promise<void> {
    if (!this._contexts[context]) {
      const rules = this._dataRules.filter(
        (x) => x.keyStrategy === 'afterInsert',
      );
  ...
  private _getContextProxy(
    dataRules: AbstractDataRuleCommand[],
  ): ProxyConstructor {
    return new Proxy(
      {},
      {
        ...
        set(target: any, prop: any, value: any) {
          ...
          dataRules
            ...
            .forEach((dataRule) =>
              dataRule.applyAfterInsert(prop, target[prop], target),
            );
          return true;
        },
      },
  ...
  public async putFile(data: KeyFileData): Promise<string> {
    try {
      ...
      const fileLink = await this.fileClientService.presignedGetObject(
        data.userUuid,
        objectName,
      );
  ...
  private _applyRulesBeforeCommit(document: object): object {
    const rules = this._dataRules.filter(
      (dataRule) => dataRule.keyStrategy === 'beforeCommit',
    );
