...
export class XpathLoopSelection extends AbstractLoopSelection {
	...
	private _getLoopElements(command: AbstractCommand, resolver: DataContextResolver): LoopElement[] {
		...
		while (true) {
			...
			const docRef = selectors.length === 0 ? 'document': selectors[selectors.length - 1].variableName;
	...
	public querySelectorAll(command: AbstractCommand, resolver: DataContextResolver): string {
