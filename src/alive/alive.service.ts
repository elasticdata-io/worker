...
export class AliveService {
  ...
  public async handleCron(): Promise<void> {
    if (
      this.envConfiguration.USE_ISOLATION_MODE ||
      !this.envConfiguration.USE_ALIVE_PROBE
    ) {
      return;
    }
    const WORKER_TYPE = this.configService.get<string>('WORKER_TYPE');
    const WORKER_MANAGER_URL =
      this.configService.get<string>('WORKER_MANAGER_URL');
    const FROM_CONTAINER_KEY =
      this.configService.get<string>('FROM_CONTAINER_KEY');
