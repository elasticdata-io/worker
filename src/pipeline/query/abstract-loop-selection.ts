...
export abstract class AbstractLoopSelection {
  ...
  public abstract querySelectorAll(
    command: AbstractCommand,
    resolver: DataContextResolver,
  ): string;
  public abstract querySelector(
    command: AbstractCommand,
    resolver: DataContextResolver,
  ): string;

  protected _getSelectorsWithIndex(
    command: AbstractCommand,
    resolver: DataContextResolver,
  ): (string | number)[] {
    const cmdSelector = command.getSelector();
    const selectors = cmdSelector.split(
      AbstractLoopSelection.SPLIT_LOOP_INDEX_PATTERN,
    );
    return selectors
  ...
  protected getElIndex(
    command: AbstractCommand,
    loopIndexDsl: string,
    resolver: DataContextResolver,
  ): number {
    const contextName = loopIndexDsl.replace(
      AbstractLoopSelection.LOOP_INDEX_PATTERN,
      '$1',
    );
