...
export class BrowserProvider extends IBrowserProvider {
  ...
  constructor(@inject(ROOT_TYPES.PipelineIoc) private _ioc: PipelineIoc) {
    ...
    this._commandAnalyzer = this._ioc.get<AbstractCommandAnalyzer>(
      TYPES.AbstractCommandAnalyzer,
    );
    this._pool = this._ioc.get<Pool<{ page: Page; browser: Browser }>>(
      ROOT_TYPES.BrowserPool,
    );
    this._userInteractionInspector = this._ioc.get<UserInteractionInspector>(
      ROOT_TYPES.UserInteractionInspector,
    );
  }
  ...
  public async execute(
    ...
    try {
      if (this._browser.hasBeenDestroyed() && this._browser.hasBeenAborted()) {
        console.info(
          `SKIP COMMAND: ${command.constructor.name} because browser has been stopped`,
        );
        return;
      }
      if (typeof inDataContext === 'string') {
        const dataContextResolver = this._ioc.get<DataContextResolver>(
          ROOT_TYPES.DataContextResolver,
        );
        dataContextResolver.copyContext(inDataContext, [command]);
      }
      if (
        typeof inPageContext === 'number' ||
        inPageContext instanceof AbstractCommand
      ) {
      ...
      const captchaService = this._ioc.get<CaptchaService>(
        ROOT_TYPES.CaptchaService,
      );
    ...
    return new Promise((resolve, reject) => {
      ...
      this._waitCompletedInterval = setInterval(() => {
        ...
        const workingMinutes =
          (endOn.getTime() - startOn.getTime()) / 1000 / 60;
  ...
  private _isPollCompleted(): boolean {
    console.log(
      `this._pool.pending = ${this._pool.pending}, this._pool.borrowed = ${this._pool.borrowed}`,
    );
