...
export abstract class AbstractCommand implements Selectable {
  ...
  constructor(ioc: PipelineIoc) {
    ...
    this.browserProvider = ioc.get<BrowserProvider>(
      ROOT_TYPES.IBrowserProvider,
    );
    this.queryProviderFactory = ioc.get<QueryProviderFactory>(
      ROOT_TYPES.QueryProviderFactory,
    );
    this.dataContextResolver = ioc.get<DataContextResolver>(
      ROOT_TYPES.DataContextResolver,
    );
    this.pageContextResolver = ioc.get<PageContextResolver>(
      ROOT_TYPES.PageContextResolver,
    );
    this.eventBus = ioc.get<EventBus>(ROOT_TYPES.EventBus);
    this.ioc = ioc;
    this._commandAnalyzer = this.ioc.get<AbstractCommandAnalyzer>(
      ROOT_TYPES.AbstractCommandAnalyzer,
    );
    this._userInteractionInspector = this.ioc.get<UserInteractionInspector>(
  ...
  public async getKey(): Promise<string> {
    ...
    if (keyCommand) {
      await this.browserProvider.execute(keyCommand, {
        silent: true,
        inDataContext: this,
      });
