...
} from '../../documentation/specification';
...
export class CommandFactory extends ICommandFactory {
  constructor(
    ...
    @inject(ROOT_TYPES.DataContextResolver)
    private _dataContextResolver: DataContextResolver,
    @inject(ROOT_TYPES.PageContextResolver)
  ...
  public appendUuidToCommands(commandsJson: string): string {
    this.appendUuidToCommands = (): string => {
      throw new SystemError(
        `the function 'appendUuidToCommands' cannot be called more than once`,
      );
    };
  ...
  public createOpenTabRuntimeCommand(config: {
    ...
    openTabRuntimeCommand.materializedUuidPath =
      openTabCommand.materializedUuidPath;
  ...
  private _creteCommand(config: any): AbstractCommand {
    ...
    switch (cmd.toLocaleLowerCase()) {
      ...
      case 'loop':
        ...
        loop.commands = this.createChainCommands(
    ...
    if (command) {
      for (const [key, value] of Object.entries(config)) {
        ...
        if (!ignoreKeys) {
          ...
          if (!exists) {
            const allowProps =
              CommandSpecification.getAssignableProperties(command);
            throw new SystemError(
  ...
  private _initDataContext(commands: AbstractCommand[]) {
    commands.forEach((command) =>
      this._dataContextResolver.setRootContext(command),
    );
  }

  private _initPageContext(commands: AbstractCommand[]) {
    commands.forEach((command) =>
      this._pageContextResolver.setRootContext(command),
    );
  }
}
