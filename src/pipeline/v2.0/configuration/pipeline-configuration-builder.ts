...
@injectable()
export class PipelineConfigurationBuilder
  implements IPipelineConfigurationBuilder
{
  private _json: string;
  private readonly SUPPORTING_VERSIONS = ['2.0'];

  constructor(
    @inject(TYPES.ICommandFactory) private _commandFactory: ICommandFactory,
  ) {}
  ...
  buildFromJson(json: any): IPipelineConfigurationBuilder {
    ...
    this._commandsJson =
      this._commandFactory.appendUuidToCommands(commandsJson);
    this._commands = this._commandFactory.createChainCommands(
      this._commandsJson,
    );
