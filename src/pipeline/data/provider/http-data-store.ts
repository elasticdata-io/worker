...
export class HttpDataStore extends AbstractStore {
  ...
  constructor(
    ...
    @inject(TYPES.AbstractCommandAnalyzer)
    commandAnalyzer: AbstractCommandAnalyzer,
  ) {
   ...
   * @param command
   */
  async attachJpegFile(
    jpegScreenshotBuffer: Buffer,
    command: AbstractCommand,
  ): Promise<string> {
   ...
   * @param command
   */
  async attachWebpFile(
    webpScreenshotBuffer: Buffer,
    command: AbstractCommand,
  ): Promise<string> {
