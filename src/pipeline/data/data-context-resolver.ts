...
export class DataContextResolver {
  ...
  private setChildrenDataContext(
    commands: AbstractCommand[],
    commandsContext: string,
  ) {
    commands.forEach((command) => command.setDataContext(commandsContext));
  }
    ...
    if (contextName) {
      ...
      if (contexts.length !== 2) {
        throw new SystemError(
          `contextName: '${contextName}' is not correct with context: ${context}`,
        );
      }
