...
import {
  ...
  UserInteractionSettingsConfiguration,
} from './configuration/settings-configuration';
...
export class PipelineBuilder implements IPipelineBuilder {
  ...
  constructor(
    @inject(ROOT_TYPES.IPipelineConfigurationBuilder)
    private _pipelineConfigurationBuilder: IPipelineConfigurationBuilder,
  ...
  async build(): Promise<PipelineProcess> {
    ...
    this._ioc.bind<Driver>(ROOT_TYPES.Driver).toConstantValue(driver);
    this._registerUserInteraction(settings.userInteraction);
    try {
      const pipelineConfiguration =
        this._pipelineConfigurationBuilder.buildFromJson(this._pipelineJson);
      // const browserProvider = new BrowserProvider(this._ioc);
      const browserProvider = this._ioc.get<IBrowserProvider>(
      ...
      this._pipelineProcess = new PipelineProcess(
        commandsJson,
        commands,
        dataRules,
        browserProvider,
  ...
  private _registerUserInteraction(
    userInteraction: UserInteractionSettingsConfiguration,
  ) {
    this._ioc
      .bind<UserInteractionSettingsConfiguration>(
