...
export class UserInteractionInspector {
  ...
  private get needWatchCommands(): AbstractCommand[] {
    ...
    this._needWatchCommands =
      this.commandFactory.createCommands(watchCommandsJson);
    return this._needWatchCommands;
  }
  ...
  constructor(@inject(ROOT_TYPES.PipelineIoc) private _ioc: PipelineIoc) {
    this._userInteraction = this._ioc.get<UserInteractionSettingsConfiguration>(
      TYPES.UserInteractionSettingsConfiguration,
    );
    ...
    this._commandFactory = this._ioc.get<ICommandFactory>(
  ...
  private _initListeners() {
    this._eventBus.on(
      PipelineCommandEvent.BEFORE_EXECUTE_NEXT_COMMAND,
      (command: AbstractCommand) => this.onBeforeExecuteNextCommand(command),
    );
    this._eventBus.on(UserInteractionEvent.EXECUTE_CMD, (dto: ExecuteCmdDto) =>
      this.onExecuteCmd(dto),
    );
    this._eventBus.on(
      UserInteractionEvent.DISABLE_USER_INTERACTION_MODE,
      (dto: DisableUserInteractionStateDto) =>
        this.onDisableInteractionMode(dto),
    );
  }

  private async onBeforeExecuteNextCommand(
  ...
  private async onExecuteCmd(dto: ExecuteCmdDto): Promise<void> {
    const commands = this._commandFactory.createCommands(
      JSON.stringify(dto.commands),
    );
    try {
      for (const command of commands) {
        const pageContext = parseInt(dto.pageContext, 10);
        console.log(`EXECUTE COMMANDS IN PAGE CONTEXT: ${pageContext}`);
        await this.browserProvider.execute(command, {
          silent: true,
          inPageContext: pageContext,
        });
      }
    } catch (e) {
      console.error(e);
    ...
    await this._eventBus.emit(
      UserInteractionEvent.UPDATE_USER_INTERACTION_MODE,
      data,
    );
  }

  private async _getJpegScreenshotLink(
    command: AbstractCommand,
  ): Promise<string> {
    const screenshotBuffer = await this._driver.getScreenshot(command, {
      quality: 70,
    });
    return await this._dataStore.attachJpegFile(screenshotBuffer, command);
  }

  private async _executeWatchCommand(
    command: AbstractCommand,
    context: AbstractCommand,
  ): Promise<boolean> {
    try {
      await this.browserProvider.execute(command, {
        inPageContext: context,
        silent: true,
      });
    } catch (e) {
      return false;
  ...
  public async checkNeedInteractionMode(
    executedCommand: AbstractCommand,
  ): Promise<number> {
    for (const [index, watchCommand] of Object.entries(
      this.needWatchCommands,
    )) {
      const pageContext =
        this._pageContextResolver.resolveContext(executedCommand);
      const watchCommandIndex = parseInt(index);
      if (this._enabledInteractionMode[pageContext] === watchCommandIndex) {
        continue;
      }
      const successful = await this._executeWatchCommand(
        watchCommand,
        executedCommand,
      );
      if (successful) {
  ...
  private async enableUserInteractionMode(
    command: AbstractCommand,
    watchCommandIndex: number,
  ): Promise<void> {
    ...
    const jpegScreenshotLink = await this._dataStore.attachJpegFile(
      screenshotBuffer,
      command,
    );
    let pageElements = [];
    ...
    console.log(
      `ENABLE_USER_INTERACTION_MODE in pageContext: ${pageContext}, currentUrl: ${currentUrl}`,
    );
    const timeout = UserInteractionInspector.DEFAULT_WAIT_MINUTES * 60 * 1000;
    await this._driver.wait(timeout, 1000, () =>
